...
public:
	...
	*/
	...
	string calculate()
	{
		...
		int score = 0;
		int result;
		for (iter = testScores.begin(); iter < testScores.end(); iter++) score += *iter;
		result = score / testScores.size();

		if (result/10 == 10 || result/10 == 9) return "O";
		else if (result/10 == 8) return "E";
		else if (result/10 == 7) return "A";
